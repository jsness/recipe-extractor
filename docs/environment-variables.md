...
| `EXTRACTOR` | `anthropic` | LLM fallback backend to use: `anthropic` or `openai` |
| `ANTHROPIC_API_KEY` | _(optional)_ | Anthropic API key; if unset, LLM fallback is disabled and only JSON-LD extraction is used |
| `ANTHROPIC_MODEL` | `claude-sonnet-4-6` | Anthropic model ID |
| `ANTHROPIC_TIMEOUT_SECONDS` | `45` | Request timeout for Anthropic calls |
| `OPENAI_API_KEY` | _(optional)_ | OpenAI API key; if unset, LLM fallback is disabled and only JSON-LD extraction is used |
